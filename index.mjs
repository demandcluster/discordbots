...
const { bot, horde } = await import(`./bots/${params.tag}.js`).then(module => module?.default).catch((e) => {
...
const fetchData = async (left = 0) => {
    ...
    for (const request of requests) {
        try {
            const { data } = await axios.get(`https://horde.aivo.chat/api/v2/generate/text/status/${request.id}`);

            if (data.faulted) {
                await db('requests').where({ id: request.id }).update({ status: RequestType.Faulted, editedAt: db.fn.now() });
                console.log('Fault encountered during text generation.',JSON.stringify(data));
                continue;
            }
...
client.on(Events.MessageCreate, async (message) => {
    ...
    try {
        const { data } = await axios.post(`https://horde.aivo.chat/api/v2/generate/text/async`, payload, {
            headers: {
