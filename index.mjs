...
const { bot, horde } = await import(`./bots/${params.tag}.js`)
  .then((module) => module?.default)
  .catch((e) => {
    console.log(
      "The bot config file has not been loaded.\nMake sure it's in the directory and run the command with the argument.\n" +
        params.tag
    );
    process.exit();
  });
...
const fetchData = async (left = 0) => {
  ...
  const requests = await db
    .select("id", "user", "channel", "message")
    .from("requests")
    .where({ status: RequestType.Processing })
    .catch(() => []);
  ...
  for (const request of requests) {
    try {
      const { data } = await axios.get(
        `https://horde.aivo.chat/api/v2/generate/text/status/${request.id}`
      );

      if (data.faulted) {
        await db("requests")
          .where({ id: request.id })
          .update({ status: RequestType.Faulted, editedAt: db.fn.now() });
        console.log(
          "Fault encountered during text generation.",
          JSON.stringify(data)
        );
        continue;
      }
      ...
      if (data.done) {
        ...
        await db("requests")
          .where({ id: request.id })
          .update({ status: RequestType.Finished, editedAt: db.fn.now() });
  ...
  setTimeout(
    fetchData,
    Math.max(0, 4000 - (Date.now() - startTime)),
    requests.length
  );
};
...
client.on(Events.MessageCreate, async (message) => {
  if (
    ...
    message.content.match(/@(here|everyone)/i)
  )
    return;
...
<|model|>{{post}}
  ...
  try {
    const { data } = await axios.post(
      `https://horde.aivo.chat/api/v2/generate/text/async`,
      payload,
      {
        headers: {
    ...
    console.log(
      "The new request was not added for processing due to an error."
    );
  }
});
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.commandName === "stats") {
    const { users, messages } = await db("users")
      .select(
        db.raw(
          "COUNT(id) AS users, (SELECT COUNT(id) FROM messages WHERE bot = 1) AS messages"
        )
      )
      .first();
    const stats = new EmbedBuilder()
      ...
      .setDescription(
        `I already sent ${messages} message(s) to ${users} user(s) on ${client.guilds.cache.size} server(s).`
      )
      .addFields(
...
client.on(Events.ClientReady, async () => {
  ...
  const { users, messages } = await db("users")
    ...
    .first();
  console.log(
    `${params.tag} has already sent ${messages} message(s) to ${users} user(s) on ${client.guilds.cache.size} server(s).`
  );
